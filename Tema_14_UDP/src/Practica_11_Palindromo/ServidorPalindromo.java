package Practica_11_Palindromo;
...
public class ServidorPalindromo {
	...
	public static void main(String[] args) {
		...
		try {
			...
			StringBuilder invertido = new StringBuilder(mensaje);
			
			
			String	palabraInvertida = invertido.reverse().toString();
			
			boolean esPalindromo = true;
			for(int i =0; i<mensaje.length();i++) {
				
				if(mensaje.charAt(i) != palabraInvertida.charAt(i) ) {
					esPalindromo = false;
				}
			}
			...
			flujoSalida.writeBoolean(esPalindromo); //Enviamos el booleano
			flujoSalida.close();
