package Practica_23_BuscarPalabra;
...
public class ServidorBuscar {
	...
	public static void main(String[] args) {
		try {
			...
			String [] arrayPalabras = (String[]) flujoEntrada.readObject();
			String palabraEnviada = flujoEntrada.readUTF();
			
			
			
			
			boolean exsistePalabra = false;
			
			
			
			
			for(String palabra : arrayPalabras) {
				if(palabra.equalsIgnoreCase(palabraEnviada)) {
					exsistePalabra = true;
				}
			...
			flujoSalida.writeBoolean(exsistePalabra); //Enviamos el booleano
			flujoSalida.close();
